...
export class InputController {
    ...
    /**
    * Generate object from list in string, elements separated with ', ' 
    */
    generateObjectsFromString() {
        let arrayFr = this.inputListFr.split(", ");
        let arrayEn = this.inputListEn.split(", ");
        if (this.inputListFr !== '' && this.inputListEn !== '' && arrayEn.length == arrayFr.length) {
            for (let obj in arrayFr) {
                this.createObject();
                this.elementList[obj].tag = arrayEn[obj].toUpperCase().replace(' ', '');
                this.elementList[obj].labelMaleFr = this.capitalizeFirstLetter(arrayFr[obj]);
                this.elementList[obj].labelFemaleFr = this.capitalizeFirstLetter(arrayFr[obj]);
                this.elementList[obj].labelMaleEn = this.capitalizeFirstLetter(arrayEn[obj]);
                this.elementList[obj].labelFemaleEn = this.capitalizeFirstLetter(arrayEn[obj]);
                this.elementList[obj].defaultLabelFr = this.capitalizeFirstLetter(arrayFr[obj]);
                this.elementList[obj].defaultLabelEn = this.capitalizeFirstLetter(arrayEn[obj]);
            }
        }
    }
